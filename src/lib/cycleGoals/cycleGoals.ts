...
const cycleGoalsSchema = z.object({
  ...
  projectsGoal: z.string().refine((value) => Number(value) >= 0, {
    message: "A meta de projetos não pode ser negativa.",
  }),
  membersGoal: z.string().refine((value) => Number(value) >= 0, {
    message: "A meta de membros não pode ser negativa.",
  }),
  npsGoal: z
    .string()
