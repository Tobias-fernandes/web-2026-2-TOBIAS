    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className={cn('m-0 font-display font-bold', !active && 'line-through')}>
            {event.title}
          </p>
      ...
      {!active && (
        <p className="mt-2 mb-0 text-sm font-semibold text-vermelho">Cancelado</p>
      )}
      ...
      <dl className="m-0 mt-3 flex flex-col gap-1">
        ...
        {!derived && <Detail label="Marcado por">{memberName(event.createdBy)}</Detail>}
      </dl>
      ...
      {derived ? (
        ...
        editable && (
          <div className="mt-3 flex flex-wrap gap-1 border-t border-linha pt-2.5">
            <button type="button" className={ACTION} onClick={() => onEdit(event)}>
              Editar
            </button>
