      ...
      <div className="env grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
        ...
        <dl className="m-0 divide-y divide-linha border-y border-linha">
          {DIRECTORATE_PITCH.map((item) => (
            <div key={item.directorate} className="grid gap-2 py-5 sm:grid-cols-[145px_1fr] sm:gap-6">
              <dt className="text-sm font-semibold">{DIRECTORATE_LABELS[item.directorate]}</dt>
              <dd className="m-0 text-sm leading-relaxed text-tinta-suave">{item.text}</dd>
            </div>
          ))}
        </dl>
      </div>
