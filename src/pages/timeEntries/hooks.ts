...
function buildRows(
  ...
  for (const entry of entries) {
    ...
    row.hoursByDate[entry.date] =
      (row.hoursByDate[entry.date] ?? 0) + entry.hours;
...
function resolveMember(
  ...
  if (seesEveryone)
    return { memberId: roster[0]?.id ?? chosenId, unlinked: false };
...
export function useTimesheetPage(): TimesheetPageState {
  ...
  return {
    ...
    weekEntries,
    rows: useMemo(
      () => buildRows(weekEntries, projectName),
      [weekEntries, projectName],
    ),
