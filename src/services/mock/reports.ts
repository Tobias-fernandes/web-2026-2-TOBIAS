...
export const mockReports: ReportService = {
  ...
  async workloadByMember(scope: ReportScope = {}): Promise<MemberWorkload[]> {
    ...
    const [allMembers, cycleMemberships, entries, openAllocations] =
      await Promise.all([
        members.list(),
        memberships.listBy({ cycleId }),
        timeEntries.listBy({ from, to }),
    ...
    const weeksFor = (startsAt: IsoDate) => {
      const start =
        startsAt > (from ?? startsAt) ? startsAt : (from ?? startsAt);
  ...
  async funnel(scope: ReportScope = {}): Promise<FunnelSummary> {
    ...
    const reasons = [
      ...new Set(lost.map((deal) => deal.lossReason ?? "other")),
    ];
    ...
    return {
      ...
      averageDaysToClose:
        closed.length > 0
          ? Math.round(
              sum(closed, (deal) =>
                daysBetween(deal.createdAt, deal.closedAt ?? deal.createdAt),
              ) / closed.length,
            )
          : null,
      ...
      bySource: sources
        .map((source) => {
          const fromSource = cycleDeals.filter(
            (deal) => deal.source === source,
          );
          const wonFromSource = fromSource.filter(
            (deal) => deal.stage === "won",
          );
          return {
            source,
      ...
      lossReasons: reasons
        .map((reason) => ({
          reason,
          count: lost.filter((deal) => (deal.lossReason ?? "other") === reason)
            .length,
        }))
  ...
  async cashFlow(scope: ReportScope = {}): Promise<CashFlowSummary> {
    ...
    const of = (kind: "receivable" | "payable", settled: boolean) =>
      ledger.filter(
        (line) => line.kind === kind && (line.paidAt !== null) === settled,
      );
